...
- (UITextView *)textFied {
    if (!_textFied) {
        _textFied = [[UITextView alloc] init];
        _textFied.textColor = [UIColor blackColor];
...
- (UILabel *)limitTextLengthLabel
{
    if(!_limitTextLengthLabel)
    {
        ...
        _limitTextLengthLabel.text = @"150";
...
{
    //如果有文字则隐藏  无文字则显示
    self.placeholderLab.hidden = self.textFied.hasText;
    
    if(_limitTextLength > 0)
    {
        NSInteger textLength = self.textFied.text.length;
        
        if (textLength > self.limitTextLength) {
            
            self.limitTextLengthBlock();
            self.textFied.text = [self.textFied.text substringToIndex:self.limitTextLength];
            
        }
        
        self.limitTextLengthLabel.text = [NSString stringWithFormat:@"%d",self.limitTextLength - (int)self.textFied.text.length];
    }
...
- (void)layoutSubviews
{
    ...
    self.textFied.frame = CGRectMake(5,0,self.frame.size.width - 10, self.frame.size.height - 20);
